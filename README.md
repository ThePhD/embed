...
```cpp
implementation-magic __builtin_init_array<_Type>(size_t __resource_name_len, _CharTy* __resource_name_ptr, bool __null_terminated, size_t __expected_decoded_resource_size, quote-delimited-base64-data);
```

Where:
- `_Type` is the element type of the array that the built-in is initializing that satisfies `std::is_trivial_v<_Type>` with `true`;
- `_CharTy` is one of `char`, `wchar_t`, `char8_t`, `char16_t`, or `char32_t`;
...
#### `__builtin_data_cache`

Generated by a call to the preprocessor directive `#depend`, this contains purely the bits of data ready to be re-consumed by the front end and stored in a string literal cache. It's primary purpose is to make distributed build systems have an easier time processing single files, as well as minimal test case reproduction and similar.


```cpp
void __builtin_data_cache<_Type>(size_t __resource_name_len, _CharTy* __resource_name_ptr, bool __null_terminated, size_t __expected_decoded_resource_size, quote-delimited-base64-data);
```
...
This will fill an internal compiler cache with the decoded data from the base64 argument. Whenever a `__builtin_embed` call is made that references the `__resource_name` referenced by a call made to this built-in, the compiler uses this pre-cached data.
